...
@Component({
  ...
  templateUrl: './taxpayer-user.component.html'
})
export class TaxpayerUserComponent implements OnInit {
  ...
  buildForm(): void {
    this.taxpayerUserForm = this.formBuilder.group({
      email: [this.taxpayerUser.email, Validators.compose([
        ...
        fullName: [this.taxpayerUser.fullName,
        [Validators.required]
      ],
  ...
  submitTaxpayerUserForm(taxpayerUser: TaxpayerUser, isValid: boolean) {
    ...
    if (isValid) {
      ...
      this.taxpayerUserService.registerTaxpayerUser(this.taxpayerUser)
